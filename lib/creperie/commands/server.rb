...
module Creperie
  module Commands
    class Server < Base
      ...
      option ['-I', '--include'],   'PATH', "Add paths (colon-separated) to Crêpe's $LOAD_PATH", attribute_name: '_include' do |o|
        $LOAD_PATH.unshift(*o.split(':'))
      end
      option ['-r', '--require'],   'LIBRARY', 'Require a library before Crêpe runs', attribute_name: '_require' do |o|
        require o
      end

      option ['-d', '--debug'],     :flag, 'Turn on debug output ($DEBUG = true)'
      option ['-w', '--warn'],      :flag, 'Turn on warnings ($-w = true)'
